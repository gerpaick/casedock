from __future__ import annotations

import uuid_utils.compat as uuid
from django.conf import settings
from django.db import migrations, models
from django.db.backends.base.schema import BaseDatabaseSchemaEditor


def backfill_user_and_public_id(
    apps: migrations.state.StateApps, schema_editor: BaseDatabaseSchemaEditor
) -> None:
    User = apps.get_model("core", "User")
    if not User.objects.exists():
        return
    first_user_id = User.objects.earliest("id").id
    FocusAssignment = apps.get_model("focus", "FocusAssignment")
    for assignment in FocusAssignment.objects.all():
        assignment.user_id = first_user_id
        assignment.public_id = uuid.uuid7()
        assignment.save(update_fields=["user", "public_id"])


class Migration(migrations.Migration):

    dependencies = [
        ("focus", "0001_initial"),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.RemoveConstraint(
            model_name="focusassignment",
            name="uniq_focus_assignment_per_case_per_day",
        ),
        migrations.RemoveConstraint(
            model_name="focusassignment",
            name="uniq_focus_assignment_slot_per_day",
        ),
        migrations.AddField(
            model_name="focusassignment",
            name="public_id",
            field=models.UUIDField(
                db_index=True,
                default=uuid.uuid7,
                editable=False,
                null=True,
            ),
        ),
        migrations.AddField(
            model_name="focusassignment",
            name="user",
            field=models.ForeignKey(
                null=True,
                on_delete=models.deletion.CASCADE,
                related_name="focus_assignments",
                to=settings.AUTH_USER_MODEL,
            ),
        ),
        migrations.RunPython(backfill_user_and_public_id, migrations.RunPython.noop),
        migrations.AlterField(
            model_name="focusassignment",
            name="public_id",
            field=models.UUIDField(
                db_index=True,
                default=uuid.uuid7,
                editable=False,
                unique=True,
            ),
        ),
        migrations.AlterField(
            model_name="focusassignment",
            name="user",
            field=models.ForeignKey(
                on_delete=models.deletion.CASCADE,
                related_name="focus_assignments",
                to=settings.AUTH_USER_MODEL,
            ),
        ),
        migrations.AddConstraint(
            model_name="focusassignment",
            constraint=models.UniqueConstraint(
                fields=("user", "focus_date", "case"),
                name="uniq_focus_assignment_per_case_per_day",
            ),
        ),
        migrations.AddConstraint(
            model_name="focusassignment",
            constraint=models.UniqueConstraint(
                fields=("user", "focus_date", "role", "order"),
                name="uniq_focus_assignment_slot_per_day",
            ),
        ),
    ]
