...
def backfill_user_and_public_id(
    ...
    SourceLink = apps.get_model("sources", "SourceLink")
    for link in SourceLink.objects.all():
        link.user_id = first_user_id
        link.public_id = uuid.uuid7()
        link.save(update_fields=["user", "public_id"])
